...
<head>
    ...
    <script>
        let antype = '${tableForm.antype}';
        $(document).ready(function () {
            let riverPatrol = new RiverPatrol();
            if (antype == 'area') {
                riverPatrol.initAreaGrid();
                riverPatrol.refreshAreaTable();
            } else if (antype == 'river') {
                riverPatrol.initRiverGrid();
                riverPatrol.refreshRiverTable();
            }
...
<div class="container" style="width:100%; height:100%">
    <div class="row" style="height: 90px; padding: 12px 5px 0 5px;">
        <div style="height: 100%; background-color: #FFFFFF; padding: 20px 0">
            <form:form id="tbaleForm" class="form-inline" modelAttribute="tableForm">
                <form:input type="hidden" id="moduleCode" path="moduleCode"></form:input>
                <div class="col-lg-4 col-lg-offset-1">
                    <c:if test="${tableForm.moduleCode eq 'county'}">
                    ...
                    <c:if test="${tableForm.moduleCode eq 'city'}">
                ...
                <div class="col-lg-4">
                    ...
                    <input id="month" name="month" type="text" readonly="readonly" maxlength="20"
                           ...
                           value="<fmt:formatDate value="${tableForm.month}" pattern="yyyy-MM"/>"
    ...
    <div id="area_row" class="row" style="height: 720px; padding: 12px 8px 8px 8px;">
        <div class="col-lg-12" style="height: 100%; background-color: #FFFFFF;">
            <c:if test="${tableForm.antype eq 'area'}">
                <table class="table-bordered" id="area_table"></table>
            </c:if>
            <c:if test="${tableForm.antype eq 'river'}">
                <table class="table-bordered" id="river_table"></table>
            </c:if>
        </div>
    </div>
</div>
