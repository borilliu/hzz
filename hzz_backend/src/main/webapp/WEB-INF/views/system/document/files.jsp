...
<body style="overflow-y: hidden" scroll="no">
	<t:formvalid formid="formobj" layout="div" dialog="true"
		...
		<input type="hidden" id="docId"
			<c:if test="${not empty doc }">value="${doc.id }"</c:if>>
		<fieldset class="step">
			<div class="form">
			...
			<div class="form">
				<t:upload name="fiels" buttonText="上传文件"
					uploader="jeecgFormDemoController.do?saveFiles&documentId=${doc.id}"
					extend="" id="file_upload" formData="documentTitle"></t:upload>
			</div>
			<div class="form" id="filediv" style="height: 50px">
				<c:if test="${not empty attachment }">
					<div class="row" style="margin-left: 20px;">
						${attachment.attachmenttitle }.${attachment.extend }</div>
				</c:if>
			</div>
		</fieldset>
	</t:formvalid>
	<input type="hidden" id="attachment"
		<c:if test="${not empty attachment }">value="1"</c:if> />
