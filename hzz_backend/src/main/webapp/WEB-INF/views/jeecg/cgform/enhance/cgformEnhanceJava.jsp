			...
			<tr>
				...
				<td class="value"><select id="buttonCode" name="buttonCode"
					datatype="*" style="width: 200px;">
						...
						<option value="update"
							<c:if test="${cgformEnhanceJavaPage.buttonCode=='update'}">selected="selected"</c:if>>update</option>
						<option value="delete"
							<c:if test="${cgformEnhanceJavaPage.buttonCode=='delete'}">selected="selected"</c:if>>delete</option>
						<c:forEach items="${buttonList}" var="button">
							<option value="${button.buttonCode }"
								<c:if test="${button.buttonCode==cgformEnhanceJavaPage.buttonCode}">selected="selected"</c:if>>${button.buttonCode}</option>
						</c:forEach>
				</select> <span class="Validform_checktip"></span></td>
			</tr>
			<tr>
				<td align="center" width="150px"><label class="Validform_label">
						<t:mutiLang langKey="common.event" />:
				</label></td>
				<td class="value"><input type="radio" name="event"
					value="start"
					${empty cgformEnhanceJavaPage.event or cgformEnhanceJavaPage.event eq 'start' ? 'checked' : '' } />
				<t:mutiLang langKey="common.begin" /> <input type="radio"
					name="event" value="end"
					${empty cgformEnhanceJavaPage.event or cgformEnhanceJavaPage.event eq 'end' ? 'checked' : '' } />
				<t:mutiLang langKey="common.end" /> <span class="Validform_checktip"></span>
				</td>
			</tr>
			<tr>
				<td align="center" width="150px"><label class="Validform_label">
						<t:mutiLang langKey="common.type" />:
				</label></td>
				<td class="value"><t:dictSelect field="cgJavaType" type="radio"
						typeGroupCode="enhanceType" hasLabel="false"
						defaultVal="${cgformEnhanceJavaPage.cgJavaType==null?'spring':(cgformEnhanceJavaPage.cgJavaType)}"></t:dictSelect><span
					class="Validform_checktip"></span></td>
			</tr>
			<tr>
				<td align="center" width="150px"><label class="Validform_label">
						<t:mutiLang langKey="common.value" />:
				</label></td>
				<!--update-end--Author:luobaoli  Date:20150701 for：取消非空校验-->
				<td class="value"><input id="cgJavaValue" name="cgJavaValue"
					type="text" style="width: 300px; height: 26px"
					value="${cgformEnhanceJavaPage.cgJavaValue}" /><span
					class="Validform_checktip"></span></td>
				<!--update-end--Author:luobaoli  Date:20150701 for：取消非空校验-->
			</tr>
			<tr>
				<td align="center" width="150px"><label class="Validform_label">
						<t:mutiLang langKey="common.iseffect" />:
				</label></td>
