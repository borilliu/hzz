...
<div class="row">
	<div class="col-md-12 layout-header">
		<button id="addBtn_JformOrderTicket2" type="button"
			class="btn btn-default">添加</button>
		<button id="delBtn_JformOrderTicket2" type="button"
...
<div style="margin: 0; background-color: white; overflow: auto;">
	<!-- Table -->
	<table id="jformOrderTicket2_table"
		...
		<tbody id="add_jformOrderTicket2_table">
			<c:if test="${fn:length(jformOrderTicket2List)  <= 0 }">
				<tr>
					...
					<td><input name="jformOrderTicket2List[0].ticketCode"
						maxlength="100" type="text" class="form-control"
						style="width: 120px;" datatype="*" ignore="checked" /></td>
					<td><input name="jformOrderTicket2List[0].tickectDate"
			...
			<c:if test="${fn:length(jformOrderTicket2List)  > 0 }">
				<c:forEach items="${jformOrderTicket2List}" var="poVal"
					varStatus="stuts">
					<tr>
						<td><input style="width: 20px;" type="checkbox" name="ck" />
							<input name="jformOrderTicket2List[${stuts.index }].id"
							type="hidden" value="${poVal.id }" /> <input
							name="jformOrderTicket2List[${stuts.index }].fckId" type="hidden"
							value="${poVal.fckId }" /></td>
						...
						<td><input
							name="jformOrderTicket2List[${stuts.index }].ticketCode"
							maxlength="100" type="text" class="form-control"
							style="width: 120px;" datatype="*" ignore="checked"
							value="${poVal.ticketCode }" /></td>
						<td><input
							name="jformOrderTicket2List[${stuts.index }].tickectDate"
							maxlength="10" type="text" class="form-control"
							onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"
							style="background: url('plug-in/ace/images/datetime.png') no-repeat scroll right center transparent; width: 120px;"
