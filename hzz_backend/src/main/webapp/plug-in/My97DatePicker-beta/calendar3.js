...
My97DP.prototype =
    {
        ...
        , splitDate: function (J, C, Q, E, B, G, F, K, L) {
            ...
            if (J && J.loadDate) $ = J;
                ...
                if (J != "") {
                    C = C || $dp.dateFmt;
                    var H, P = 0, O, A = /yyyy|yyy|yy|y|MMMM|MMM|MM|M|dd|d|%ld|HH|H|mm|m|ss|s|DD|D|WW|W|w/g,
                        _ = C.match(A);
                    A.lastIndex = 0;
                    if (L) O = J.split(/\W+/);
                    else {
            ...
            function N(A, $) {
                ...
                for (var B = 0;
                     B < 12;
                     B++) if (_[B].toLowerCase() == $.substr(0, _[B].length).toLowerCase()) return B + 1;
                return -1
            }
        }
        ,
        ...
        doExp: function (s) {
            ...
            for (var i = 0;
                 i < ps.length;
                 i++) s = s.replace("%" + ps.charAt(i), this.getP(ps.charAt(i), null, $tdt));
        ...
        ,
        _fd: function () {
            var E, C, D, K, A, H = new sb(), F = $lang.aWeekStr, G = $dp.firstDayOfWeek, I = "", $ = "",
                _ = new DPDate($dt.y, $dt.M, $dt.d, 0, 0, 0), J = _.y, B = _.M;
        ...
        _fy: function (_, B, A) {
            ...
            $.a(this.minDate.y < B ? "class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown='if(event.preventDefault)event.preventDefault();
                event.cancelBubble = true;
            $c._fy(0, "+(B-10)+", "+A+")
            '":"class='
            invalidMenu
            '");
            $.a(">\u2190</td><td class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown=\"hide($d.yD);
            $d.yI.blur();
			\">\xd7</td><td "
        )
            ;
            $.a(this.maxDate.y > B + 10 ? "class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown='if(event.preventDefault)event.preventDefault();
                event.cancelBubble = true;
            $c._fy(0, "+(B+10)+", "+A+")
            '":"class='
            invalidMenu
        ...
        _fillQS: function (C, A) {
            ...
            if (!C) _.a("<div style=\"float:right;
            cursor:pointer\" onclick=\"hide($d.qsDivSel);
			\">\xd7</div>"
        )
            ;
            _.a("</td></tr>");
            for (var D = 0;
                ...
                ;
                _.a("&nbsp;
                "+this.getDateStr(null,B[D]));
                _.a("</td></tr>")
            } else _.a("<tr><td class='menu'>&nbsp;
                < /td></
            tr > ");
            _.a("</table>");
...
function _tab(M) {
    ...
    if ($dp.enableKeyboard && isShow) {
        ...
        if (Q >= 37 && Q <= 40) {
            ...
            var D = H.nextCtrl;
            for (var R = 0;
                 R < $dp.focusArr.length;
                 R++) if (D.disabled == true || D.offsetHeight == 0) D = D.nextCtrl;
            else break;
            if ($c.currFocus != D) {
                $c.currFocus = D;
                D.focus()
