package com.saili.hzz.web.graphreport.service.impl.core;
		...
		while (m.find()) {
			...
			if(el.indexOf(":") != -1) {
				...
				*/
				...
				Set<Entry<String,Object>> entry1=params.entrySet();
				...
				while(itentry1.hasNext()){
					...
					if (el.equals(key)){
						//sql = sql.replace(oel, el + condValue.getValue().toString());
						if (oel1.length()>0) oel1=oel1+" and ";
						oel1=oel1+el + value.toString();
						itentry1.remove();
					}
				}
				/*
				for (Object entry : params.entrySet()) {
					Entry<String, Object> condValue=(Entry<String,Object>)entry;
					if(condValue != null) {
						if (el.equals(condValue.getKey())){
							//sql = sql.replace(oel, el + condValue.getValue().toString());
							if (oel1.length()>0) oel1=oel1+" and ";
							oel1=oel1+el + condValue.getValue().toString();
							params.remove(condValue.getKey());
						}
					}
					
				}*/
				if (oel1.length()==0) oel1="1=1";
				sql = sql.replace(oel, oel1);
			}
		}
		return sql;
	}
