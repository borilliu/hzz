	...
	<div class="bd3">
		<#list config_queryList as x>
			...
			<#if x['search_mode']=="group">
				<input type="text" name="${x['field_name']}_begin"  style="width: 94px;margin-bottom: 5px;margin-top: 5px;"  <#if x['field_type']=="Date">class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"</#if> />
				<span style="display:-moz-inline-box;display: none;width: 8px;text-align:right;">~</span>
				...
				<#if x['field_type']=="Date">
					<script type="text/javascript">
						...
						$(function () {
							...
							$(":input[name='${x['field_name']}_begin']").val(date1);
							$(":input[name='${x['field_name']}_end']").val(date1);
		...
		<#if (config_queryList?size > 0)>
			<span style="display:-moz-inline-box;display:inline-block;margin-left: 20px;">
				<a href="#"  id="searchBtn" style="display: inline-block;margin-right: 10px;width: 50px;"><img style="top: 3px;position: relative;" src="plug-in/graphreport/css/images/search.png"> 查询</a> &nbsp;
				<a href="#"  id="resetBtn" style="display: inline-block;margin-right: 10px;width: 50px;"><img style="top: 3px;position: relative;" src="plug-in/graphreport/css/images/reset.png"> 重置</a>
			</span>
		</#if>	 
	</div>
</div>
...
$(function() {
	...
	$("#exportExcel").click(function() {
		window.location = "slCrossController.do?datagridGraph&export=1&" + $.param(getParam());
	});
	
	//重置
	$("#resetBtn").click(function() {
		//window.location = "slCrossController.do?list&id=${config_id}";
		//location=location;
...
function show() {
	//显示趋势图
	$.post("slCrossController.do?datagridGraph", getParam(), function(data) {
		log(data);
		data = eval(data).rows;
		data1 = data; 
