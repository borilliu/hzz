...
import com.saili.hzz.web.system.pojo.base.TSNoticeReadUser;
import com.saili.hzz.web.system.pojo.base.TSRole;
import com.saili.hzz.web.system.service.NoticeAuthorityRoleServiceI;
import com.saili.hzz.web.system.service.NoticeAuthorityUserServiceI;
import com.saili.hzz.web.system.service.NoticeService;
...
public class NoticeController extends BaseController{
	...
	@RequestMapping(params = "checkNoticeCode")
	@ResponseBody
	public ValidForm checkNoticeCode(String param, HttpServletRequest request) {
		ValidForm v = new ValidForm();
		v.setInfo("验证通过！");
		v.setStatus("y");
		String noticeCode=oConvertUtils.getString(request.getParameter("param"));

		int count = noticeService.countByNoticeCode(noticeCode);
...
//			systemService.saveOrUpdate(readUser);
	...
	public AjaxJson doAdd(TSNotice tSNotice, HttpServletRequest request) {
		...
		String testFile2 = oConvertUtils.getString(request.getParameter("testFile2"));
		
		//if (StringUtil.isEmpty(testFile1))tSNotice.setAttachmentDoc(testFile1);
		//if (StringUtil.isEmpty(testFile2))tSNotice.setTitlePicture(testFile2);
		try{
			TSUser u = ResourceUtil.getSessionUser();
			tSNotice.setCreateUser(u.getId());
			if (StringUtil.isEmpty(tSNotice.getId())){
