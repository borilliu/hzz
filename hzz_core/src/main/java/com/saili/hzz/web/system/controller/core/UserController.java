...
import com.saili.hzz.core.util.ListtoMenu;
...
import com.saili.hzz.core.util.RoletoJson;
import com.saili.hzz.core.util.SetListSort;
...
import com.saili.hzz.web.system.pojo.base.InterroleEntity;
...
//			}
	 ...
	 * @param response
	 */
	@RequestMapping(params = "datagridByDivisionCode")
	@ResponseBody
	public List<TSUser> datagridByDivisionCode(TSUser user, String divisionCode, HttpServletRequest request, HttpServletResponse response) {
		String divisionCode1 = request.getParameter("divisionCode");
		//查询机构下用户列表
//		MiniDaoPage<TSUser> list = departAuthGroupDao.getUserByDepartCode(dataGrid.getPage(), dataGrid.getRows(),orgCode , user);
		List<TSUser> results = userService.selectByDivisionCode(divisionCode1);
		return results;
	}
...
**/	
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			//users.setTSLDivision(user.getTSLDivision());
			users.setUserDutys(user.getUserDutys());
			users.setUserOrderBy(user.getUserOrderBy());
			TSLDivisionEntity division = systemService.findUniqueByProperty(TSLDivisionEntity.class, "code", user.getTSLDivision().getCode());
			users.setTSLDivision(division);
			
			TSLUserTypeEntity tslUserTypeEntity = systemService.findUniqueByProperty(TSLUserTypeEntity.class, "code", user.getTSLUserTypeEntity().getCode());
			users.setTSLUserTypeEntity(tslUserTypeEntity);

			//users.setTSLUserTypeEntity(user.getTSLUserTypeEntity());
