...
public class ProcessDefCache {
	...
	public static ProcessDefinition get(String procDefId) {
		CacheServiceI cache = ApplicationContextUtil.getContext().getBean(CacheServiceI.class);
		ProcessDefinition pd = (ProcessDefinition)cache.get(ACT_CACHE, ACT_CACHE_PD_ID_ + procDefId);
		if (pd == null) {
...
//			pd = (ProcessDefinitionEntity) ((RepositoryServiceImpl) repositoryService).getDeployedProcessDefinition(pd);
			...
			if (pd != null) {
				cache.put(ACT_CACHE, ACT_CACHE_PD_ID_ + procDefId, pd);
			}
		}
		return pd;
	}
