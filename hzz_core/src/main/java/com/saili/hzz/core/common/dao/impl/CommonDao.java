package com.saili.hzz.core.common.dao.impl;


import com.saili.hzz.core.common.dao.ICommonDao;
import com.saili.hzz.core.common.dao.IGenericBaseCommonDao;
...
import com.saili.hzz.core.extend.swftools.SwfToolsUtil;
import com.saili.hzz.core.extend.template.DataSourceMap;
...
import com.saili.hzz.core.entity.TSLBaseRiverLakePatrolEventPhoto1;
...
public class CommonDao extends GenericBaseCommonDao implements ICommonDao, IGenericBaseCommonDao {
	...
	public List<TSRole> listRole(TSUser user) {
		List<TSRoleUser> sRoleUser = findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
		return sRoleUser.stream().map(TSRoleUser::getTSRole).collect(Collectors.toList());
	}
...
//					reflectHelper.setMethodValue(uploadFile.getByteField(), StreamUtils.InputStreamTOByte(mf.getInputStream()));
				...
				try{
					//这个是第一开始做的
					Object object1 = uploadFile.getObject1();
					if (object1!=null){
						ReflectHelper reflectHelper1 = new ReflectHelper(object1);
						String sttempName=reflectHelper1.getMethodValue("tempName").toString();
						if ("archivesDoc".equals(sttempName)){
							reflectHelper1.setMethodValue("archivesDoc", path + myfilename);	
						}
						if ("policyDoc".equals(sttempName)){
							reflectHelper1.setMethodValue("policyDoc", path + myfilename);	
						}
						if ("publicSignsDoc".equals(sttempName)){
							reflectHelper1.setMethodValue("publicSignsDoc", path + myfilename);	
						}
						saveOrUpdate(object1);
					}
				}catch(Exception ex){
				...
				try{
					//这是第二次改的
					CgUploadEntity object2=(CgUploadEntity)object;
					if (object2!=null){
						String cgformName=object2.getCgformName();
						String cgformId=object2.getCgformId();
						String cgformField=object2.getCgformField();
						if ("t_sl_base_river_lake_patrol_event_photo".equals(cgformName)){
							TSLBaseRiverLakePatrolEventPhoto photo =new TSLBaseRiverLakePatrolEventPhoto();
							TSLBaseRiverLakePatrolEvent event=(TSLBaseRiverLakePatrolEvent) findUniqueByProperty(TSLBaseRiverLakePatrolEvent.class,"id",cgformId);
							photo.setTslBaseRiverLakePatrolEvent(event);
							photo.setPhotoDate(new Date());
							photo.setCgUploadId(reflectHelper.getMethodValue("id").toString());
							photo.setPhoto(reflectHelper.getMethodValue(uploadFile.getRealPath()).toString());
							saveOrUpdate(photo);
							
						}
						if ("t_sl_base_river_lake_patrol_event_photo1".equals(cgformName)){
							TSLBaseRiverLakePatrolEventPhoto1 photo =new TSLBaseRiverLakePatrolEventPhoto1();
							TSLBaseRiverLakePatrolEvent event=(TSLBaseRiverLakePatrolEvent) findUniqueByProperty(TSLBaseRiverLakePatrolEvent.class,"id",cgformId);
							photo.setTslBaseRiverLakePatrolEvent(event);
							photo.setPhotoDate(new Date());
							photo.setCgUploadId(reflectHelper.getMethodValue("id").toString());
