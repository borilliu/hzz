package com.saili.hzz.core.servlet;
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (!item.isFormField() && item.getName().length() > 0) {
					...
					// com.saili.hzz.core.util.LogUtil.info(item.getName()+"=="+UploadTool.createPhotoID()+suffixName+"=="+UploadTool.FormetFileSize(item.getSize())+"savePath"+savePath);
