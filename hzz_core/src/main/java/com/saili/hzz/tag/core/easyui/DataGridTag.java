...
import com.saili.hzz.tag.vo.easyui.ColumnValue;
import com.saili.hzz.tag.vo.easyui.DataGridColumn;
import com.saili.hzz.tag.vo.easyui.DataGridUrl;
import com.saili.hzz.tag.vo.easyui.OptTypeDirection;
...
import com.saili.hzz.core.util.SysThemesUtil;
import com.saili.hzz.core.util.oConvertUtils;
import com.saili.hzz.tag.core.factory.BootstrapTableComponent;
import com.saili.hzz.tag.core.factory.ComponentFactory;
...
import com.saili.hzz.web.cgform.entity.config.CgSubTableVO;
import com.saili.hzz.web.cgform.service.config.CgFormFieldServiceI;
import com.saili.hzz.web.cgform.util.PublicUtil;
...
public class DataGridTag extends TagSupport {
	...
	private boolean showFooter = true;// 定义是否显示行底（如果是做统计表格，这里可以显示总计等）。
...
//        log.info("================================ DataGridTag 开始时间:"+sdf.format(new Date())+"==============================");
		...
		try {
			...
			if("bootstrap-table".equals(component)){
				...
				String content = componentFactory.invoke("/com/saili/hzz/tag/core/factory/ftl/component/bootstrapTable.ftl", getDataGridTag());
...
//         log.info("================================ DataGridTag 耗时:"+(end-start)+"ms==============================");
	...
	public StringBuffer jqGrid(){
		...
		sb.append("<link href=\""+basePath+"/plug-in/hplus/css/bootstrap.min.css?v=3.3.6\" rel=\"stylesheet\">");
		sb.append("<link type=\"text/css\" rel=\"stylesheet\" href=\""+basePath+"/plug-in/hplus/css/plugins/jqgrid/ui.jqgrid.css\">");
		sb.append("<script src=\""+basePath+"/plug-in/hplus/js/jquery.min.js\"></script>");
		...
		sb.append("<script src=\""+basePath+"/plug-in/hplus/js/bootstrap.min.js\"></script>");
		sb.append("<script src=\""+basePath+"/plug-in/hplus/js/plugins/peity/jquery.peity.min.js\"></script>");
		sb.append("<script src=\""+basePath+"/plug-in/hplus/js/plugins/jqgrid/i18n/grid.locale-cn.js\"></script>");
		sb.append("<script src=\""+basePath+"/plug-in/hplus/js/plugins/jqgrid/jquery.jqGrid.min.js\"></script>");

		sb.append("<script src=\""+basePath+"/plug-in/tools/datagrid_2_jqgrid.js\"></script>");
		sb.append("<style>");
...
//		}
	...
	public StringBuffer datatables() {
		...
		sb.append("<link href=\""+basePath+"/plug-in/hplus/css/plugins/dataTables/dataTables.bootstrap.css\" rel=\"stylesheet\">");
		sb.append("<script src=\""+basePath+"/plug-in/hplus/js/plugins/dataTables/jquery.dataTables.js\"></script>");
	...
	public StringBuffer end() {
		...
		if(btnCls!=null && btnCls.indexOf("bootstrap")==0){
			sb.append("<link rel=\"stylesheet\" href=\""+basePath+"/plug-in/bootstrap/css/bootstrap-btn.css\" type=\"text/css\"></link>");
		}

		boolean hasGroup = hasGroup();
		if(hasGroup){
			sb.append("<link rel=\"stylesheet\" href=\""+basePath+"/plug-in/tools/css/optgroup.css\" type=\"text/css\"></link>");
		}
		...
		if(!treegrid && isShowSubGrid){
			sb.append("<script type=\"text/javascript\" src=\""+basePath+"/plug-in/easyui/extends/datagrid-detailview.js\"></script>");
		}
...
//					}
		...
		if(hasQueryColum(columnList)&&isShowSearch==true){
			sb.append("<input  id=\"columsShow\" type=\"image\" src=\""+basePath+"/plug-in/easyui/themes/default/images/accordion_collapse.png\" onclick=\"isShowBut()\">");
		}
		...
		if(hasQueryColum(columnList) && "group".equals(getQueryMode())){
			...
			sb.append("<span><img style=\"margin-top:-3px;vertical-align:middle;\" src=\""+basePath+"/plug-in/easyui/themes/icons/ti.png\"  title=\"提示：模糊查询通配符: *，多个关键字用半角逗号 ',' 隔开！\" alt=\"提示：模糊查询通配符: *，多个关键字用半角逗号 ',' 隔开！\" /></span>");
		...
		if(blink){
			sb.insert(0, "<link rel=\"stylesheet\" href=\""+basePath+"/plug-in/Validform/css/style.css\" type=\"text/css\">" +
		...
		if("group".equals(getQueryMode())){
			...
			for (DataGridColumn col : columnList) {
				if (col.isQuery()) {
					...
					if(i==0){
						int iwidth=74;
						iwidth=iwidth+(col.getTitle().length() - 4)*15;
						sb.append("<span style=\"vertical-align:middle;display:-moz-inline-box;display:inline-block;width:"+iwidth+"px;text-align:right;text-overflow:ellipsis;-o-text-overflow:ellipsis; overflow: hidden;white-space:nowrap; \" title=\""+col.getTitle()+"\">"+"&nbsp;&nbsp;&nbsp;"+col.getTitle()+"："+ (col.getTitle().length()<4?"&nbsp;&nbsp;&nbsp;":"") + "</span>");
					}else{
...
//	                                    + " onMouseOver=\"toolTip('<img src='+value+'>')\" onMouseOut=\"toolTip()\" "
	...
	public String getNoAuthOperButton(){
		...
		//com.saili.hzz.core.util.LogUtil.info("----getNoAuthOperButton-------"+sb.toString());
		return sb.toString(); 
	}
	...
	public StringBuffer aceStyleTable() {
		...
		width = (width == null) ? "auto" : width;
		height = (height == null) ? "auto" : height;
//		sb.append("<link rel=\"stylesheet\" href=\""+basePath+"/plug-in/easyui/themes/metro/main.css\" />");
		sb.append("<script type=\"text/javascript\">");
		sb.append("$(function(){  storage=$.localStorage;if(!storage)storage=$.cookieStorage;");
		sb.append(this.getNoAuthOperButton());
		if (treegrid) {
...
//		appendLine(sb,"				{'fieldId':'office_Phone','fieldName':'办公电话'},");
		...
		//已在baseTag中引入
//		appendLine(sb,"<script type=\"text/javascript\" src=\""+basePath+"/plug-in/jquery/jquery.cookie.js\" ></script>");
//		appendLine(sb,"<script type=\"text/javascript\" src=\""+basePath+"/plug-in/jquery-plugs/storage/jquery.storageapi.min.js\" ></script>");
	...
	private void addSuperQuery(StringBuffer sb,String buttonSytle,List<DataGridColumn> columnList) {
		...
		String superQuery = free.parseTemplate("/com/saili/hzz/tag/ftl/superquery.ftl", mainConfig);
	...
	private void addSuperQueryBootstrap(StringBuffer sb,String buttonSytle,List<DataGridColumn> columnList) {
		...
		mainConfig.put("tableName", name);
		mainConfig.put("valueList", columnValueList);
		String superQuery = free.parseTemplate("/com/saili/hzz/tag/ftl/superqueryBootstrap.ftl", mainConfig);
		appendLine(sb,superQuery);
	}
	...
	public void getFilter(StringBuffer sb){
		if (this.isFilter()) {
			...
			String superQuery = free.parseTemplate("/com/saili/hzz/tag/ftl/filter.ftl", mainConfig);
			appendLine(sb,superQuery);
		}
	}
